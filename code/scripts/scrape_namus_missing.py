...
Uses the NamUs public JSON API to fetch all missing person cases.
API endpoints:
  - POST /api/CaseSets/NamUs/MissingPersons/Search (search/list)
  - GET  /api/CaseSets/NamUs/MissingPersons/Cases/{id} (case details)
...
SEARCH_ENDPOINT = f"{BASE_URL}/api/CaseSets/NamUs/MissingPersons/Search"
CASE_ENDPOINT = f"{BASE_URL}/api/CaseSets/NamUs/MissingPersons/Cases"
OUTPUT_FILE = "data/raw/namus_missing.json"
OUTPUT_JSONL = "data/raw/namus_missing.jsonl"
...
def search_cases(session, state=None, take=100, skip=0):
    """
    Search for missing person cases using the NamUs API.
    ...
    """
    # Exact projections from browser capture
    payload = {
        "take": take,
        "skip": skip,
        "projections": [
...
def test_api(session):
    ...
    print("Testing NamUs Missing Persons API")
    ...
    print(f"✅ Search API working. Total missing persons: {count}")
    ...
    if cases:
        ...
        if case_id:
            details = get_case_details(session, case_id)
            if details:
                name = f"{details.get('firstName', '')} {details.get('lastName', '')}"
                print(f"✅ Case Detail API working. Sample: {name.strip()}")
                return True
...
def main():
    parser = argparse.ArgumentParser(description="Scrape NamUs Missing Persons database")
    ...
    print("=" * 60)
    print("NamUs Missing Persons Scraper")
